...
class FigureWidget(QWidget):
    ...
    def applyZoom(self, xmin, xmax, ymin, ymax):
        ...
        # Capture current state before applying new zoom
        previous_limits = []
        for ax in self.figure.axes:
            previous_limits.append((ax.get_xlim(), ax.get_ylim()))
        self._zoom_history.append(previous_limits)
        
        # Apply new limits to all axes
        for ax in self.figure.axes:
            ax.set_xlim(xmin, xmax)
            ax.set_ylim(ymax, ymin)  # Invert y-axis for depth
    ...
    def undoZoom(self):
        ...
        if len(self._zoom_history) > 0:
            ...
            for ax, (xlim, ylim) in zip(self.figure.axes, prev_limits):
                ax.set_xlim(xlim)
                ax.set_ylim(ylim)
            self.canvas.draw()
            # Update current_zoom_limits to None since we reverted
...
class WellLogViewer(QMainWindow):
    ...
    def initUI(self):
        ...
        self.share_y_axis_action = QAction("UnLink Y Axis", self, checkable=True)
    ...
    def undoZoom(self):
        ...
        if self.sync_zoom_enabled:
            # Undo zoom for all widgets in sync mode
            for widget in self.figure_widgets.values():
                if widget.undoZoom():
                    # If undo was successful, apply the new zoom limits to all widgets
                    self.sync_zoom_limits = widget.current_zoom_limits
                    for other_widget in self.figure_widgets.values():
                        other_widget.applyZoom(*self.sync_zoom_limits)
                    break  # Exit after the first successful undo
        else:
            # Undo zoom for the currently selected well in single zoom mode
            if self.current_single_zoom_well:
                widget = self.figure_widgets[self.current_single_zoom_well]
                if widget.undoZoom():
                    self.single_zoom_limits = widget.current_zoom_limits
    ...
    def draw_well_top_connections(self):
        """Draw connections with proper top axis and handling of deselection"""
        selected_wells = self.get_ordered_visible_wells()
        if len(selected_wells) < 2:
            return

        # Create dictionary of well tops
        top_dict = defaultdict(dict)
        for well in selected_wells:
            if well in self.well_tops:
                for top, md in self.well_tops[well]:
                    if top in self.selected_top_names:
                        top_dict[top][well] = md

        # Get reference dimensions from first well
        ref_well = self.figure_widgets[selected_wells[0]]
        well_height = ref_well.height()

        # Find top axis position from well plots
        top_axis_pos = self.get_top_axis_position(ref_well)

        # Create connections between adjacent wells
        for i in range(len(selected_wells) - 1):
            well1, well2 = selected_wells[i], selected_wells[i + 1]

            # Create connection widget
            connection_widget = QWidget()
            connection_widget.setFixedHeight(well_height)
            connection_widget.setFixedWidth(60)  # Narrow width

            # Configure figure with no padding
            fig = Figure()
            fig.subplots_adjust(left=0, right=1, bottom=0, top=1)
            canvas = FigureCanvas(fig)
            canvas.setFixedSize(60, well_height)
            ...
            well1_pos = self.get_widget_position(well1)
            if well1_pos is not None:
                self.figure_layout.insertWidget(well1_pos + 1, connection_widget)

            # Configure axes
            ax = fig.add_subplot(111)
            ax.set_position([0, 0, 1, 1])  # Use full space

            # Set matching y-limits
            y_min1, y_max1 = self.figure_widgets[well1].figure.axes[0].get_ylim()
            y_min2, y_max2 = self.figure_widgets[well2].figure.axes[0].get_ylim()
            ax.set_ylim(max(y_min1, y_min2), min(y_max1, y_max2))

            # Configure top x-axis to match well plots
            ...
            # Draw connections
            self.draw_connection_lines(ax, top_dict, well1, well2)
    ...
    def get_top_axis_position(self, well_widget):
        ...
        return 0.95  # Default if not found
